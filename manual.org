...
files pulled in with ~include~, ~bazel-repo-mode~ for REPO.bazel
files, ~bazelrc-mode~ for =.bazelrc= configuration files, ~bazelignore-mode~ for
...
REPO.bazel files, and Starlark files is closely related, the corresponding modes
derive from a parent mode, ~bazel-mode~.  Collectively, these modes are called
“Bazel modes” in the rest of this manual.
