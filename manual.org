# Copyright 2021-2024, 2026 Google LLC
#
...
#+FINDEX: bazel-repo-mode
#+FINDEX: bazel-vendor-mode
#+FINDEX: bazelrc-mode
#+FINDEX: bazelignore-mode
#+FINDEX: bazeliskrc-mode
#+FINDEX: bazel-starlark-mode
The package provides several major modes for editing Bazel-related files:
~bazel-build-mode~ for BUILD files, ~bazel-workspace-mode~ for WORKSPACE files,
~bazel-module-mode~ for MODULE.bazel files as well as =*.MODULE.bazel= segment
files pulled in with ~include~, ~bazel-repo-mode~ for REPO.bazel files,
~bazel-vendor-mode~ for VENDOR.bazel files, ~bazelrc-mode~ for =.bazelrc=
...
#+FINDEX: bazel-mode
Since the syntax for BUILD files, WORKSPACE files, MODULE.bazel files,
REPO.bazel files, VENDOR.bazel files, and Starlark files is closely related, the
corresponding modes derive from a parent mode, ~bazel-mode~.  Collectively,
these modes are called “Bazel modes” in the rest of this manual.
...
#+CINDEX: Speedbar
#+FINDEX: speedbar
When =bazel.el= is loaded, rule targets defined in Bazel BUILD and WORKSPACE
files will show up in the Speedbar.  For more information about Speedbar, see
...
For rules integrated with Bazel’s code coverage framework, =bazel.el=
...
#+FINDEX: bazel-show-consuming-target
To find out which Bazel rule target consumes the file that the current buffer
visits, run the command ~bazel-show-consuming-target~.  It will search the
corresponding BUILD file for a rule target definition that has the visited file
in its list of source files and jump there.
